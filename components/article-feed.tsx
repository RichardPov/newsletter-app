    ...
    const handleLike = async (articleId: string) => {
        try {
            ...
            setArticles(articles.map(a =>
                a.id === articleId ? { ...a, isLiked: result.isLiked } : a
            ))

            toast.success(result.isLiked ? "Added to liked" : "Removed from liked")
        } catch (error) {
            toast.error("Failed to update like status")
        }
    }

    const handleGenerateSocial = (article: ArticleData) => {
                            ...
                            <Card key={article.id} className="flex flex-col">
                                ...
                                <CardFooter className="flex gap-2">
                                    ...
                                    <Button
                                        size="icon"
                                        variant="outline"
                                        onClick={() => handleGenerateSocial(article)}
                                    >
